...
1. **Drag a scroll view** a few points, if the focused field's screen (or sheet) has one above the keyboard. The drag only starts from a spot clear of the scroll view's controls, and never on the screen behind a sheet. Web views are never dragged.
