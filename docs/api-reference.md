...
**Covered elements (iOS).** Like a Playwright click, a tap auto-waits for the element to be enabled and then for nothing to be drawn over it. If another element or the software keyboard covers the point it would tap, the tap waits for the cover to go away and then fails instead of tapping the cover: `Element is covered by the keyboard …` or `Element is covered by button "Overlay" …`. When the keyboard covers only part of the element, the tap lands on the visible part. `doubleTap()`, `longPress()`, and the tap that `type()`, `clear()` and `focus()` use to focus a field follow the same rules. If the element goes away while its cover is being waited out, the action fails as not found instead of tapping whatever replaced it. If the keyboard is in the way, dismiss it first (`device.hideKeyboard()`, or submit the field). Android does not have this check yet (PILOT-362).
