...
Any other value — in the string form or as `mode` in the object form, at the root, in a project's `use`, or in `test.use()` — fails config loading with `trace must be one of 'off', 'on', … (got "…")`, so a typo in an untyped `.js`/`.mjs` config cannot silently turn recording off. `video` is checked the same way.
...
cannot use (any N > 1 when every project runs on that device's platform and
there is more than one file to spread) is refused. In a config spanning Android and iOS, the pin applies only to the
